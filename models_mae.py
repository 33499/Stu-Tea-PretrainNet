...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_delete.shape[1], 1)
        x_v = torch.cat([x_visible, mask_tokens], dim=1)  # no cls token
        x_v = torch.gather(x_v, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle 

        # 对x_delete添加position
        visible_tokens = self.visible_token.repeat(x.shape[0], ids_keep.shape[1], 1)
        x_m = torch.cat([visible_tokens, x_delete], dim=1)  # no cls token
        x_m = torch.gather(x_m, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle 
        ...
        x_v = torch.gather(x_v, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        x_m = torch.gather(x_m, dim=1, index=ids_delete.unsqueeze(-1).repeat(1, 1, D))
        ...
        return x_v,x_m, mask, ids_restore, ids_keep, ids_delete, x_delete_pos_embeds, x_visible_pos_embeds

    def forward_crossAttention(self, x_visible,x_delete, x_delete_pos_embeds, x_visible_pos_embeds):
        # 转化weicross_attention需要输入的格式
        query_embed = x_delete_pos_embeds.permute(1, 0, 2)  # query的位置编码信息
    ...
    def infoNece_loss(self,pre_delete,gt_delete):
        ...
        negative = [torch.einsum('bld->lbd', gt_delete[negative_index]) for negative_index in negative_indexs]
    ...
    def simsiam_loss(self,pre_delete,gt_delete):
        ...
        z1 = z1.detach()
        z2 = z2.detach()
        loss = -(self.Sim_criterion(p1, z2).mean() + self.Sim_criterion(p2, z1).mean()) * 0.5
        return loss
    ...
    def forward(self, imgs, mask_ratio=0.75):
        # (1,50,1024),(1,196)
        x_visible,x_delete, mask, ids_restore, ids_keep, ids_delete, x_delete_pos_embeds, x_visible_pos_embeds = self.forward_encoder(
            imgs, mask_ratio)
        ...
        loss_1,pre_imgs = self.pre_loss(pre_delete.permute(1, 0, 2), imgs, ids_delete)

        loss_2 = self.simsiam_loss(pre_delete.permute(1, 0, 2), gt_delete)
